...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        void Start()
        {
            ...
            _screens = pageCount == 0 ? _screensContainer.childCount : pageCount;

            _lerp = false;

            _containerSize = containerSize == 0 ? (int)_screensContainer.gameObject.GetComponent<RectTransform>().offsetMax.x : containerSize;
            ...
            if (_screens > 0)
            {
                ...
                {
                    float position = ((float)i / (float)(_screens - 1)) * _containerSize;
                    _positions.Add(new Vector3(-position, 0, 0));
                }
            }
